...
function renderContent(content) {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith('- ')) {
      const items = [];
      while (i < lines.length && lines[i].startsWith('- ')) {
        items.push(<li key={i} style={{ color: 'rgba(200,210,255,0.65)', lineHeight: 1.8, marginBottom: 4 }}>{lines[i].replace('- ', '')}</li>);
        i++;
      }
    ...
    // Paragraph
    result.push(<p key={i} style={{ color: 'rgba(200,210,255,0.62)', lineHeight: 1.95, marginBottom: 16, fontSize: 16 }}>{line}</p>);
    i++;
  }
  return result;
}
