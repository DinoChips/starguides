      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 5%', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 20, marginBottom: 120 }}>
          {filtered.length === 0 ? (
            ...
            return (
              <Link key={article.id} to={`/articulo/${article.id}`} style={{ textDecoration: 'none' }}>
                <div className={`sg-card reveal reveal-delay-${(i % 4) + 1}`} style={{ padding: '32px 28px', cursor: 'pointer', transition: 'all 0.3s' }}
                  onMouseEnter={e => { e.currentTarget.style.borderColor = acc + '35'; e.currentTarget.style.background = acc + '06'; e.currentTarget.style.transform = 'translateY(-4px)'; }}
                  onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(255,200,80,0.12)'; e.currentTarget.style.background = 'rgba(255,255,255,0.018)'; e.currentTarget.style.transform = 'none'; }}>
