    ...
    <div className="sg-page">
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '0 5%', position: 'relative', zIndex: 1 }}>
        ...
        <div className="reveal" style={{ background: 'rgba(255,200,80,0.03)', border: '1px solid rgba(255,200,80,0.1)', borderRadius: 10, padding: '28px 32px', marginBottom: 72, fontFamily: 'var(--font-mono)', fontSize: 13 }}>
          ...
          <p style={{ color: 'rgba(200,210,255,0.5)', lineHeight: 1.7, marginBottom: 10 }}>
            Las guías de StarGuides son técnicas, estructuradas, al punto.
          </p>
          <p style={{ color: 'rgba(200,210,255,0.5)', lineHeight: 1.7, marginBottom: 10 }}>
            El blog es diferente: proceso, errores, opiniones. Sin filtros, sin estructura perfecta.
          </p>
        ...
        <div style={{ marginBottom: 100 }}>
          {blogPosts.map((post, index) => (
            <div key={post.id} className={`reveal reveal-delay-${(index % 4) + 1}`}>
              <div
                style={{
                  padding: '40px 0', borderBottom: '1px solid rgba(255,255,255,0.04)',
                  cursor: 'pointer', transition: 'padding-left 0.3s',
                  display: 'grid', gridTemplateColumns: '56px 1fr', gap: 24, alignItems: 'start',
                }}
                onMouseEnter={e => {
                  e.currentTarget.style.paddingLeft = '12px';
                  e.currentTarget.querySelector('.post-title').style.color = '#ffd060';
                }}
                onMouseLeave={e => {
                  e.currentTarget.style.paddingLeft = '0';
                  e.currentTarget.querySelector('.post-title').style.color = 'rgba(240,240,255,0.88)';
                }}
              >
                ...
                <div>
                  ...
                  <p style={{ fontSize: 14, color: 'rgba(200,210,255,0.38)', lineHeight: 1.7, marginBottom: 18 }}>
                    {post.excerpt}
                  </p>
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>
                    {post.tags.map(tag => (
                      <span key={tag} style={{
                        ...
                        padding: '4px 10px', border: '1px solid rgba(255,200,80,0.18)',
                        color: 'rgba(255,200,80,0.55)', borderRadius: 2,
                      }}>#{tag}</span>
                    ))}
                  </div>
        ...
        <div className="reveal" style={{
          border: '1px solid rgba(255,200,80,0.14)', borderRadius: 12, padding: 'clamp(40px,6vw,64px)',
          textAlign: 'center', marginBottom: 120,
          background: 'linear-gradient(135deg,rgba(255,200,80,0.03),rgba(80,30,140,0.03))',
          ...
          <div style={{ display: 'flex', gap: 12, maxWidth: 440, margin: '0 auto', flexWrap: 'wrap', justifyContent: 'center' }}>
            <input type="email" placeholder="tu@correo.com" style={{
              flex: 1, minWidth: 220, padding: '12px 18px',
              background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,200,80,0.18)',
              borderRadius: 3, color: 'var(--text-primary)', fontSize: 14,
